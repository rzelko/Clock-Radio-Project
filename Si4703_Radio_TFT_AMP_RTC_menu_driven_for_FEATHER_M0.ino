 ...
 *  - Web radio module - in development
...
////////////////////////////////////////////   MICROCONTROLLERS  ////////////////////////////////////////////
   ...
   #define SD_CS    5
#endif
#ifdef ARDUINO_SAMD_FEATHER_M0                                     // Feather MO 
   #define STMPE_CS A3                                             // Default is pin 6 - modified 9/2/18 to accommodate VS1053 Music Maker
   #define TFT_CS   A2                                             // Default is pin 9 - modified 9/2/18 to accommodate VS1053 Music Maker
   #define TFT_DC   A1                                             // Default is pin 10 - modified 9/2/18 to accommodate VS1053 Music Maker
   #define SD_CS    A4                                             // Default is pin 5 - modified 9/2/18 to accommodate VS1053 Music Maker
#endif
#ifdef TEENSYDUINO
   #define TFT_DC   10
   #define TFT_CS   4
...
int ampPin = 11;                                                    //digital control of SHDN function (GPIO pin 11 on Feather MO)
int gain = 10;
...
const char* myWiFiHostName[6] = {"Z Radio","SomaFM","SomaFM","SomaFM","SomaFM"}; 
const char* myWiFiPath[6] = {"/z/high.mp3", "/seventies-128-mp3", "/u80s-128-mp3", "/bagel-128-mp3", "/thistle-128-mp3" }; 
const char* myWiFiName[6] = {"Z88.3 Christian","Left Coast 70's", "Underground 80's", "Bagel Radio", "Thistle Celtic Sounds" }; 
int WiFiPathIndex = 0;                                              //establish station array indexing at 0

//const char *host = "www.klove.com";
//const char *path = "/listen/player.aspx";
//const char *host = "www.jango.com";
//const char *path = "/stations/268802010/tunein";    //.aspx";
...
//  #endif
  ...
  WiFi.lowPowerMode();
  pinMode(11,INPUT_PULLUP);                                         //initialize SHDN pin for Amp (adapted for MO controller)
...
//    tft.print(measuredvbat); tft.print("v");
  ...
  void drawBatIndicator(){
    ...
    measuredvbat *= 3.3; // Multiply by 3.3V, our reference voltage
    measuredvbat /= 1024; // convert to voltage (12 bit ADC should be 4096) 
    tft.drawRect(430,3,32,12,HX8357_RED);
    tft.fillRect(462,6,3,5,HX8357_RED);    
...
//  #endif
  ...
  tft.setCursor(78, 188);                                           //put cursor in FM stereo button
  tft.setTextSize(2);                                               //set text size 2
  tft.setTextColor(HX8357_ORANGE);
  tft.println("FM STEREO");                                         //write "FM STEREO" in button
  tft.setCursor(290, 188);                                          //put cursor in MP3 button
  tft.setTextSize(2);                                               //set text size 2
  tft.setTextColor(HX8357_ORANGE);
  tft.println("MP3 PLAYER");                                        //write "MP3" in button
  tft.setCursor(78, 258);                                           //put cursor in Web Radio button
  tft.setTextSize(2);                                               //set text size 2
  tft.setTextColor(HX8357_ORANGE);
  tft.println("WEB RADIO");                                         //write "WEB RADIO" in menu button
  tft.setCursor(323, 258);                                          //put cursor in MENU button
  tft.setTextSize(2);                                               //set text size 2
  tft.setTextColor(HX8357_ORANGE);
...
//  if (p.z > MINPRESSURE || p.z < MAXPRESSURE) {                   //if touch is above threshold - not for FeatherWing TFT
  if (ts.touched()) {
//    vert = tft.height() - p.x;
//    horz = p.y;
//    
...
void startMP3Screen(){                                              // TO BE CONVERTED TO A FUNCTIONING MP3 PLAYER
...
//    while (1);  // don't do anything more
  ...
  tft.setTextColor(HX8357_ORANGE);
  tft.setTextSize(2);
//  sprintf(printBuffer, "Station:   %02d.%01d MHz", channel / 10, channel % 10); // convert station to decimal format using print buffer
  tft.setTextColor(HX8357_ORANGE);
//  tft.print(printBuffer);            //write station
...
//  Serial.println(F("Playing: You Say"));
//  musicPlayer.playFullFile("05.mp3");

  Serial.print(F("Now playing: "));Serial.println(myAlbum_1_TrackNames[myAlbum_1_Index]);
  tft.setCursor(80, 67);
  tft.setTextColor(HX8357_ORANGE);
  tft.setTextSize(2);
  sprintf(printMP3Buffer, "%02s by %02s", myAlbum_1_TrackNames[myAlbum_1_Index], myAlbum_1_Artist ); // 
  tft.print(printMP3Buffer);
//  tft.print(myAlbum_1_TrackNames[myAlbum_1_Index]);
...
//      tft.print((now.hour()<10)? "0" : "");
  ...
  Serial.println(F("Playing: Love Like This"));
  musicPlayer.startPlayingFile("07.mp3");
...
//               "Connection: close\r\n\r\n");
  ...
  tft.setTextColor(HX8357_ORANGE);
  tft.setTextSize(2);
//  sprintf(printStreamBuffer, myWiFiHost[0], " || ", myWiFiName[0] ); // 
  tft.print(myWiFiName[WiFiPathIndex]);
...
//  tft.print(printStreamBuffer);            //write station
  ...
  TS_Point p = ts.getPoint();                                       //get touch  TSPoint p = ts.getPoint();                                        //get touch
  DateTime now = rtc.now();                                         //get current time and date
